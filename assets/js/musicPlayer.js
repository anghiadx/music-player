...
class MusicPlayer {
	config() {
		// API config
		this.songAPI = "https://api.nghiane.online/music-player/view/";
	...
	updateConfig() {
		...
		if (this.isRandom) this.randomBtn.classList.add("active");
	}
	...
	updateSong() {
		...
		if (this.isPlay) this.playBtn.click();
	}
	updateSongElement() {
		// Update list song
		this.songs = $$(".song");
	...
	updateProgress(_this) {
		...
		this.songProgress.value = 0;
		if (this.isUseProgressPseudo) this.songProgressDot.style.width = "0%";
	...
	continueUpdateProgress(percentCurrent = 0) {
		if (percentCurrent) this.audio.currentTime = (percentCurrent * this.totalTime) / 100; // Tu % tinh ra so giay hien tai
		...
		this.audio.ontimeupdate = () => {
			...
			if (this.isUseProgressPseudo) this.songProgressDot.style.width = percentCurrent + "%";
	...
	handleNext() {
		this.nextBtn.onclick = () => {
			if (this.isRandom) this.handleRandom();
			else {
				this.currenIndex++;
				if (this.currenIndex >= this.SONG_LIST.length) this.currenIndex = 0;
			}
	...
	handlePrev() {
		this.prevBtn.onclick = () => {
			if (this.isRandom) this.handleRandom();
				...
				this.currenIndex--;
				if (this.currenIndex < 0) this.currenIndex = this.SONG_LIST.length - 1;
			}
	...
	handleClickSong() {
		this.songs.forEach((song, index) => {
			song.onclick = () => {
				if (index == this.currenIndex) return;
	...
	handleEventProgress() {
		...
		function mouseMoveProgress() {
			...
			if (_this.isUseProgressPseudo) _this.songProgressDot.style.width = `${_this.songProgress.value}%`;
